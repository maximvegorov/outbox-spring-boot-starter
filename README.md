...
Комбинация `(handlerType, payload_key)` должна быть уникальной среди необработанных сообщений. Попытка опубликовать сообщение с уже существующим ключом приведёт к исключению на уровне БД. Это намеренное ограничение, предотвращающее дублирующиеся события для одного и того же объекта.
...
outbox:
  ...
  poll-interval: 1m               # интервал поллера (по умолчанию: 1m)
